...
High: ${formatValue(Yh[i])}`;
    ...
    createChart() {
      this.CandlestickChart(this.formattedCompanyData, {
        date: (d) => new Date(d.Date),
        high: (d) => d.High,
        low: (d) => d.Low,
        open: (d) => d.Open,
        close: (d) => d.Close,
        xDomain: this.$store.state.companyData.map((item) => new Date(Object.keys(item))).reverse(),
        xTicks: this.$store.state.companyData
          .map((item) => new Date(Object.keys(item)))
          .filter((item, index) => {
            if (index % 7 === 0) return item;
          }),
        yDomain: [Math.min(...this.$store.state.companyData.map((item) => item[Object.keys(item)]['3. low'])), Math.max(...this.$store.state.companyData.map((item) => item[Object.keys(item)]['2. high']))],
        yLabel: '↑ Price ($)',
        width: 1200,
        height: 500,
      });
  ...
  watch: {
    //Watching for the change in our formattedStock computed property and when it changes we delete the old SVG element (our chart) and invoke createChart function to append the new SVG element (chart) with the new data coming from Vuex
    formattedCompanyData() {
  ...
  mounted() {
    //Invoking the createChart function when the component mounts
    this.createChart();
  },
};
</script>
